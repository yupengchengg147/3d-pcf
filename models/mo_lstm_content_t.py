...
class PC_MO_LSTM_TDE(nn.Module):
    def __init__(self, use_knn=True):
        super(PC_MO_LSTM_TDE, self).__init__()
        ...
        self.sa2 = SAModule(float(1/2), MLP([64+3+16, 118, 118, 128+self.te],norm='batch_norm'), r=0.4*8.5, k=16, knn=use_knn)
        self.sa3 = SAModule(float(1/2), MLP([128+3+16, 128+16, 128+16, 256+self.te],norm='batch_norm'), r=0.8*8.5, k=8, knn=use_knn)

        self.C1 = 64
        self.C2 = 128
        self.C3 = 256

        self.local_gat1 = LPT_BD_not(16, self.C1+self.te , self.C1+self.te)
        self.local_gat2 = LPT_BD_not(16, self.C2+self.te , self.C2+self.te)
        self.local_gat3 = LPT_BD_not(8, self.C3+self.te , self.C3+self.te)

        self.lstm_1 = MS_LSTM_Cell(self.C1+self.te, self.C1+self.te, hidden_size= 2*self.C1)
        self.lstm_2 = MS_LSTM_Cell(self.C2+self.te, self.C2+self.te, hidden_size= 2*self.C2)
        self.lstm_3 = MS_LSTM_Cell(self.C3+self.te, self.C3+self.te, hidden_size= 2*self.C3)
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        for i in range(len(input_xyz_list)):
            ...
            td = torch.ones_like(fea1[:,-1], dtype=torch.float32).unsqueeze(-1).to(device) * float(i+1/total_l)
            tde = self.tde(td)
            fea1 = torch.cat((fea1, tde), dim=-1)
        ...
        for i in range(1, num_pred):
            ...
            td = torch.ones_like(fea1[:,-1], dtype=torch.float32).unsqueeze(-1).to(device) * float(i+ len(input_xyz_list) /total_l)
            tde = self.tde(td)
            fea1 = torch.cat((fea1, tde), dim=-1)
...
if __name__ == "__main__":
    ...
    testmodel = PC_MO_LSTM_TDE().to(device)
