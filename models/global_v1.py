...
class PCF_GLOBAL_witht(nn.Module):
    def __init__(self, use_knn=True):
        super(PCF_GLOBAL_witht, self).__init__()
        ...
        self.gl_seq_1 = Global_Encoder(32, MLP([2*self.C1, 2*self.C1, 2*self.C1], norm=None))
        self.gl_seq_2 = Global_Encoder(16, MLP([2*self.C2, 2*self.C2, 2*self.C2], norm=None))
        self.gl_seq_3 = Global_Encoder(16, MLP([2*self.C3, 2*self.C3, 2*self.C3], norm=None))
...
if __name__ == "__main__":
    ...
    testmodel = PCF_GLOBAL_witht().to(device)
