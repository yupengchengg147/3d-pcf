...
from models.utils import Local_Point_Trans, PC_Encoder
    
class PC_forecasting_model_knn(nn.Module):
    def __init__(self, encoder_args, att_args, upsampler_args, refiner_args, npoint):

        super(PC_forecasting_model_knn, self).__init__()
        ...
        self.local_agg_model = Local_Point_Trans(self.num_neighs, 256, 256)
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        for i in range(len(encoded_fea_list)):
                f_ij_list[i,:,:] = self.local_agg_model(encoded_fea_list[i], last_fea, 
                                                        ...
                                                        en_batch, t_i=float(i/len(encoded_fea_list)), t_last=1.0)
