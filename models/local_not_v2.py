...
class LNT_2(nn.Module):
    def __init__(self, use_knn=True):
        super(LNT_2, self).__init__()

        self.sa1 = SAModule(float(1/32), MLP([3+3, 32, 32, 64],norm='batch_norm'), r=0.2*8.5, k=32, knn=use_knn)  #512 * 64
        self.sa2 = SAModule(float(1/2), MLP([64+3, 96, 96, 128],norm='batch_norm'), r=0.4*8.5, k=16, knn=use_knn)  #256 * 128
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        f1ii = self.local_gat1.gat_btw_fr(f1_list[0], f1_list[0], xyz1_list[0], xyz1_list[0], batch1)
        f2ii = self.local_gat2.gat_btw_fr(f2_list[0], f2_list[0], xyz2_list[0], xyz2_list[0], batch2)
        f3ii = self.local_gat3.gat_btw_fr(f3_list[0], f3_list[0], xyz3_list[0], xyz3_list[0], batch3)
        
        H1_1, C1_1 = self.lstm_1(H0_1, C0_1, f_back_1, f_forward_1, f1ii)
        H1_2, C1_2 = self.lstm_2(H0_2, C0_2, f_back_2, f_forward_2, f2ii)
        H1_3, C1_3 = self.lstm_3(H0_3, C0_3, f_back_3, f_forward_3, f3ii)
        ...
        for i in range(1,len(f1_list)):
            if i < len(f1_list)-2:
                ...
                f1ii = self.local_gat1.gat_btw_fr(f1_list[i], f1_list[i], xyz1_list[i], xyz1_list[i], batch1)
                f2ii = self.local_gat2.gat_btw_fr(f2_list[i], f2_list[i], xyz2_list[i], xyz2_list[i], batch2)
                f3ii = self.local_gat3.gat_btw_fr(f3_list[i], f3_list[i], xyz3_list[i], xyz3_list[i], batch3)
                #print("f1ii.shape",f1ii.shape)
                #print("f2ii.shape",f2ii.shape)
                #print("f3ii.shape",f3ii.shape)


                H1_1, C1_1 = self.lstm_1(H1_1, C1_1, f_back_1, f_forward_1, f1ii)
                H1_2, C1_2 = self.lstm_2(H1_2, C1_2, f_back_2, f_forward_2, f2ii)
                H1_3, C1_3 = self.lstm_3(H1_3, C1_3, f_back_3, f_forward_3, f3ii)
        ...
        for i in range(1, num_pred):
            ...
            f1ii = self.local_gat1.gat_btw_fr(f1_list[-2], f1_list[-2], xyz1_list[-2], xyz1_list[-2], batch1)
            f2ii = self.local_gat2.gat_btw_fr(f2_list[-2], f2_list[-2], xyz2_list[-2], xyz2_list[-2], batch2)
            f3ii = self.local_gat3.gat_btw_fr(f3_list[-2], f3_list[-2], xyz3_list[-2], xyz3_list[-2], batch3)

            H1_1, C1_1 = self.lstm_1(prev_H1_1, prev_C1_1, f_back_1, f_forward_1, f1ii)
            H1_2, C1_2 = self.lstm_2(prev_H1_2, prev_C1_2, f_back_2, f_forward_2, f2ii)
            H1_3, C1_3 = self.lstm_3(prev_H1_3, prev_C1_3, f_back_3, f_forward_3, f3ii)
            ...
            f1ii = self.local_gat1.gat_btw_fr(f1_list[-1], f1_list[-1], xyz1_list[-1], xyz1_list[-1], batch1)
            f2ii = self.local_gat2.gat_btw_fr(f2_list[-1], f2_list[-1], xyz2_list[-1], xyz2_list[-1], batch2)
            f3ii = self.local_gat3.gat_btw_fr(f3_list[-1], f3_list[-1], xyz3_list[-1], xyz3_list[-1], batch3)
