...
class PC_MO_KNN(nn.Module):
    def __init__(self, att_args, use_knn=True, local_fea_dim = 256):
        super(PC_MO_KNN, self).__init__()
        self.sa1 = SAModule(0.25, MLP([3+3, 32, 32, 64],norm='batch_norm'), r=0.2*8.5, k=64, knn=use_knn)
        self.sa2 = SAModule(0.5, MLP([64+3, 96, 96, 128],norm='batch_norm'), r=0.4*8.5, k=32, knn=use_knn)
        self.sa3 = SAModule(0.5, MLP([128+3, 128, 128, 256],norm='batch_norm'), r=0.8*8.5, k=16, knn=use_knn)
        ...
        # encoer, decoder  seq  ~monet    model -- ModuleNotFoundError
        # monet 跑一遍
        


        
        self.local_gat1 = LPT_raw(64, self.C1 , self.C1)
        self.local_gat2 = LPT_raw(32, self.C2 , self.C2)
        self.local_gat3 = LPT_raw(16, self.C3 , self.C3)
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        for i in range(len(input_xyz_list)):
            ...
            fea0 = xyz0
            fea1, xyz1, batch1 = self.sa1(fea0, xyz0, in_batch)
            f1_list.append(fea1)
        ...
        for i in range(num_pred-1):
            ...
            xyz3_list.append(xyz3)
            # print("fea3.shape",fea3.shape), print("xyz3.shape",xyz3.shape), print("batch3.shape",batch3.shape)
            pc_next = self.pred_next(xyz0,f1_list, f2_list,f3_list, xyz1_list, xyz2_list, xyz3_list, batch1, batch2, batch3, in_batch, device)
            pred_detail_xyz_list.append(pc_next)
    ...
    def pred_next(self, last_in_xyz,f1_list, f2_list,f3_list, xyz1_list, xyz2_list, xyz3_list, batch1, batch2, batch3, in_batch, device):
        ...
        for i in range(len(f1_list)):
            # self, fea_i, fea_last, xyz_i, xyz_last, batch):

            ft1_list[i] = self.local_gat1(f1_list[i], last_f1, xyz1_list[i], last_xyz1, batch1)
            ft2_list[i] = self.local_gat2(f2_list[i], last_f2, xyz2_list[i], last_xyz2, batch2)
            ft3_list[i] = self.local_gat3(f3_list[i], last_f3, xyz3_list[i], last_xyz3, batch3)
