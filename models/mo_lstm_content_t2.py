# 3行都有，f1那行加absolute time encoding
# 采样点个数与MoNet保持一致
# add time encoding just before f1, f2, f3 into lstm,
# no time encoding in f01, ..., f10, ...
...
class PC_MO_LSTM_TDE2(nn.Module):
    def __init__(self, use_knn=True):
        super(PC_MO_LSTM_TDE2, self).__init__()
        ...
        self.lstm_1 = MS_LSTM_Cell_WITHT(self.C1, self.C1, hidden_size= 2*self.C1, dim_t=16)
        self.lstm_2 = MS_LSTM_Cell_WITHT(self.C2, self.C2, hidden_size= 2*self.C2, dim_t=32)
        self.lstm_3 = MS_LSTM_Cell_WITHT(self.C3, self.C3, hidden_size= 2*self.C3, dim_t=32)


        self.fp32 = FPModule(8, MLP([512+256, 512, 384], norm='batch_norm'))
        self.fp21 = FPModule(16, MLP([384+128, 384, 256],norm='batch_norm'))
        self.fp10 = FPModule(16, MLP([256, 256, 128], norm='batch_norm'))
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        H1_1, C1_1 = self.lstm_1(H0_1, C0_1, f_back_1, f_forward_1, f1_list[i], pos=1/total_l)
        H1_2, C1_2 = self.lstm_2(H0_2, C0_2, f_back_2, f_forward_2, f2_list[i], pos=1/total_l)
        H1_3, C1_3 = self.lstm_3(H0_3, C0_3, f_back_3, f_forward_3, f3_list[i], pos=1/total_l)
        ...
        for i in range(1,len(f1_list)):
            if i < len(f1_list)-2:
                ...
                f_back_3, f_forward_3 = self.local_gat3(f3_list[i-1], f3_list[i], f3_list[i+1],
                                            ...
                                            batch3)
                H1_1, C1_1 = self.lstm_1(H1_1, C1_1, f_back_1, f_forward_1, f1_list[i], pos=(i+1)/total_l)
                H1_2, C1_2 = self.lstm_2(H1_2, C1_2, f_back_2, f_forward_2, f2_list[i], pos=(i+1)/total_l)
                H1_3, C1_3 = self.lstm_3(H1_3, C1_3, f_back_3, f_forward_3, f3_list[i], pos=(i+1)/total_l)
        ...
        for i in range(1, num_pred):
            ...
            H1_1, C1_1 = self.lstm_1(prev_H1_1, prev_C1_1, f_back_1, f_forward_1, f1_list[-2], pos=(i+len(input_xyz_list)-1)/total_l)
            H1_2, C1_2 = self.lstm_2(prev_H1_2, prev_C1_2, f_back_2, f_forward_2, f2_list[-2], pos=(i+len(input_xyz_list)-1)/total_l)
            H1_3, C1_3 = self.lstm_3(prev_H1_3, prev_C1_3, f_back_3, f_forward_3, f3_list[-2], pos=(i+len(input_xyz_list)-1)/total_l)
            ...
            H1_1, C1_1 = self.lstm_1(H1_1, C1_1, f_back_1, f_forward_1, f1_list[-1], pos=(i+len(input_xyz_list))/total_l)
            H1_2, C1_2 = self.lstm_2(H1_2, C1_2, f_back_2, f_forward_2, f2_list[-1], pos=(i+len(input_xyz_list))/total_l)
            H1_3, C1_3 = self.lstm_3(H1_3, C1_3, f_back_3, f_forward_3, f3_list[-1], pos=(i+len(input_xyz_list))/total_l)
...
if __name__ == "__main__":
    ...
    testmodel = PC_MO_LSTM_TDE2().to(device)
