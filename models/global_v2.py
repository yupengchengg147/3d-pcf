...
#  global GCN without t and tanh at last
...
class PCF_GLOBAL_not(nn.Module):
    def __init__(self, use_knn=True):
        super(PCF_GLOBAL_not, self).__init__()

        self.sa1 = SAModule(float(1/32), MLP([3+3, 32, 32, 64],norm=None), r=0.2*8.5, k=16, knn=use_knn)  #512 * 64
        self.sa2 = SAModule(float(1/2), MLP([64+3, 96, 96, 128],norm=None), r=0.4*8.5, k=16, knn=use_knn)  #256 * 128
        self.sa3 = SAModule(float(1/2), MLP([128+3, 128, 128, 256],norm=None), r=0.8*8.5, k=8, knn=use_knn) #128 * 256
        ...
        self.fp32 = FPModule(8, MLP([512+256, 512, 384], norm=None))
        self.fp21 = FPModule(16, MLP([384+128, 384, 256],norm=None))
        self.fp10 = FPModule(16, MLP([256, 256, 128], norm=None))


        self.gl_seq_1 = Global_Encoder(32, MLP([2*self.C1, 2*self.C1, 2*self.C1], norm=None), witht=False)
        self.gl_seq_2 = Global_Encoder(16, MLP([2*self.C2, 2*self.C2, 2*self.C2], norm=None), witht=False)
        self.gl_seq_3 = Global_Encoder(16, MLP([2*self.C3, 2*self.C3, 2*self.C3], norm=None), witht=False)


        self.gl_loc_1 = MLP([4*self.C1, 3*self.C1, 2*self.C1, 2*self.C1], norm=None)
        self.gl_loc_2 = MLP([4*self.C2, 3*self.C2, 2*self.C2, 2*self.C2], norm=None)
        self.gl_loc_3 = MLP([4*self.C3, 3*self.C3, 2*self.C3, 2*self.C3], norm=None)
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        gl_out_1 = self.gl_seq_1(num_samples = 64, f1_list = f1_list, xyz1_list = xyz1_list, dbatch = batch1)
        gl_out_2 = self.gl_seq_2(num_samples = 32, f1_list = f2_list, xyz1_list = xyz2_list, dbatch = batch2)
        gl_out_3 = self.gl_seq_3(num_samples = 16, f1_list = f3_list, xyz1_list = xyz3_list, dbatch = batch3)


        t_out_1 = self.gl_loc_1(torch.cat((gl_out_1, H1_1), dim=-1))
        t_out_2 = self.gl_loc_2(torch.cat((gl_out_2, H1_2), dim=-1))
        t_out_3 = self.gl_loc_3(torch.cat((gl_out_3, H1_3), dim=-1))
        ...
        # inference
        x2, pos_2, batch_2 = self.fp32(t_out_3, last_xyz3, batch3, t_out_2, last_xyz2, batch2)
        x1, pos_1, batch_1 = self.fp21(x2, pos_2, batch_2, t_out_1, last_xyz1, batch1)
...
if __name__ == "__main__":
    ...
    testmodel = PCF_GLOBAL_not().to(device)
