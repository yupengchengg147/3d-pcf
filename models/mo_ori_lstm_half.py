#model with LSTM from MoNet, bidirectional, 3行都有，local gat有t
...
class PC_MO_LSTM(nn.Module):
    def __init__(self, use_knn=True):
        ...
        self.local_gat1 = LPT_BD(16, self.C1 , self.C1)
        self.local_gat2 = LPT_BD(16, self.C2 , self.C2)
        self.local_gat3 = LPT_BD(8, self.C3 , self.C3)
    ...
    def forward(self, input_xyz_list, num_pred, device):
        ...
        f_back_1, f_forward_1  = self.local_gat1(f1_list[i], f1_list[i], f1_list[i+1], 
                                        ...
                                        batch1, t_prev=float((i)/total_l), t_cur=float((i)/total_l), t_next=float((i+1)/total_l))
        f_back_2, f_forward_2 = self.local_gat2(f2_list[i], f2_list[i], f2_list[i+1],
                                        xyz2_list[i], xyz2_list[i], xyz2_list[i+1],
                                        batch2, t_prev=float((i)/total_l), t_cur=float((i)/total_l), t_next=float((i+1)/total_l))
        f_back_3, f_forward_3 = self.local_gat3(f3_list[i], f3_list[i], f3_list[i+1],
                                        xyz3_list[i], xyz3_list[i], xyz3_list[i+1],
                                        batch3, t_prev=float((i)/total_l), t_cur=float((i)/total_l), t_next=float((i+1)/total_l))
        ...
        for i in range(1,len(f1_list)):
            if i < len(f1_list)-1:
                f_back_1, f_forward_1 = self.local_gat1(f1_list[i-1], f1_list[i], f1_list[i+1], 
                                            ...
                                            batch1, t_prev=float((i-1)/total_l), t_cur=float((i)/total_l), t_next=float((i+1)/total_l))
                f_back_2, f_forward_2 = self.local_gat2(f2_list[i-1], f2_list[i], f2_list[i+1],
                                            xyz2_list[i-1], xyz2_list[i], xyz2_list[i+1],
                                            batch2, t_prev=float((i-1)/total_l), t_cur=float((i)/total_l), t_next=float((i+1)/total_l))
                f_back_3, f_forward_3 = self.local_gat3(f3_list[i-1], f3_list[i], f3_list[i+1],
                                            xyz3_list[i-1], xyz3_list[i], xyz3_list[i+1],
                                            batch3, t_prev=float((i-1)/total_l), t_cur=float((i)/total_l), t_next=float((i+1)/total_l))
                ...
                f_back_1, f_forward_1 = self.local_gat1(f1_list[i-1], f1_list[i], f1_list[i], 
                                            ...
                                            batch1, t_prev=float((i-1)/total_l), t_cur=float((i)/total_l), t_next=float((i)/total_l))
                f_back_2, f_forward_2 = self.local_gat2(f2_list[i-1], f2_list[i], f2_list[i],
                                            xyz2_list[i-1], xyz2_list[i], xyz2_list[i],
                                            batch2, t_prev=float((i-1)/total_l), t_cur=float((i)/total_l), t_next=float((i)/total_l))
                f_back_3, f_forward_3 = self.local_gat3(f3_list[i-1], f3_list[i], f3_list[i],
                                            xyz3_list[i-1], xyz3_list[i], xyz3_list[i],
                                            batch3, t_prev=float((i-1)/total_l), t_cur=float((i)/total_l), t_next=float((i)/total_l))
                H1_1, C1_1 = self.lstm_1(H1_1, C1_1, f_back_1, f_forward_1, f1_list[i])
                H1_2, C1_2 = self.lstm_2(H1_2, C1_2, f_back_2, f_forward_2, f2_list[i])
                H1_3, C1_3 = self.lstm_3(H1_3, C1_3, f_back_3, f_forward_3, f3_list[i])
        ...
        for i in range(1, num_pred):
            ...
            f_back_1, f_forward_1 = self.local_gat1(f1_list[-2], f1_list[-1], f1_list[-1], 
                                            ...
                                            batch1, t_prev=float((i+4)/total_l), t_cur=float((i+5)/total_l), t_next=float((i+5)/total_l))
            f_back_2, f_forward_2 = self.local_gat2(f2_list[-2], f2_list[-1], f2_list[-1],
                                        xyz2_list[-2], xyz2_list[-1], xyz2_list[-1],
                                        batch2,  t_prev=float((i+4)/total_l), t_cur=float((i+5)/total_l), t_next=float((i+5)/total_l))
            f_back_3, f_forward_3 = self.local_gat3(f3_list[-2], f3_list[-1], f3_list[-1],
                                        xyz3_list[-2], xyz3_list[-1], xyz3_list[-1],
                                        batch3,  t_prev=float((i+4)/total_l), t_cur=float((i+5)/total_l), t_next=float((i+5)/total_l))
