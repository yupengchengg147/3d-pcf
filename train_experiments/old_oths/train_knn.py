...
from models.model_knn import PC_forecasting_model_knn
...
class PointCloudForecastingModel_knn(pl.LightningModule):
    def __init__(self, encoder_args, att_args, upsampler_args, refiner_kargs, train_config):
        super(PointCloudForecastingModel_knn, self).__init__()
        self.model = PC_forecasting_model_knn(encoder_args, att_args, upsampler_args, refiner_kargs, train_config['num_points'])
...
def train_model():
    ...
    train_config = {'lr': 0.5*1e-3, 'batch_size':6, 'num_pred': 1, 'beta': (0.9, 0.999), 'num_points': 4096}
    ...
    pc_fc_model = PointCloudForecastingModel_knn(encoder_args, att_args, upsampler_args, refiner_kargs, train_config)
    # pc_fc_model = PointCloudForecastingModel_2.load_from_checkpoint(checkpoint_path='checkpoints/oth/best-checkpoint-epoch=03-val_loss=9.99.ckpt', encoder_args=encoder_args, att_args=att_args, upsampler_args=upsampler_args, refiner_kargs=refiner_kargs, train_config=train_config)
    ...
    seq_logger = TensorBoardLogger("logs", name="model_no_t")
