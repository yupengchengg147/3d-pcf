...
class KittiDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return input_pc.permute(0,2,1), output_pc.permute(0,2,1)
...
### original getitem
    ...
    #         input_pc_name = paths[i]
    #         # input_pc = self.get_cloud(os.path.join(self.root, input_pc_name))
    #         input_pc, input_fea = self.get_cloud(input_pc_name)
    #         input_pc_list.append(input_pc)
    ...
    #         output_pc_name  = paths[i]
    #         # output_pc = self.get_cloud(os.path.join(self.root, output_pc_name))
    #         output_pc, _ = self.get_cloud(output_pc_name)
    #         output_pc_list.append(output_pc)
    #     # output_pc = torch.stack(output_pc_list, dim=0)
        
    #     return input_pc_list, input_fea_list, output_pc_list
...
if __name__ == "main":
    ...
    pc_seq_dataset = KittiDataset(root=datapath, npoints=4096, input_num=5, pred_num=5, seqs=['147'])
