...
class InceptionFeatureExtractor(torch.nn.Module):
    def __init__(
        ...
        **idgcn_kwargs: args
        ...
        # Layers
        self.knn = DilatedKnnGraph(k=k * max(dilations), dilation=1, hierarchical= self.hierarchical)
        self.knn1 = Dilated(k=k, dilation=1, hierarchical= self.hierarchical)
    ...
    def forward(
        ...
        if self.use_radius_graph:
            ...
            # print(x.shape)
            # print(batch.shape)
            edge_index = self.knn(x, batch=batch)  # [2, N * k * d]   # 每个点，最近的40个neighbors
        ...
        assert res.shape == x.shape, "res and x should have the same shape"

        x_idx = fps(res, batch=batch, ratio= self.radio)

        if x_idx.max() >= n:
            remaining_indices = [i for i in range(n) if i not in x_idx]
            random.shuffle(remaining_indices)
            replacement_index = 0
            for i in range(x_idx.size(0)):
                if x_idx[i] >= n:
                    x_idx[i] = remaining_indices[replacement_index]
                    replacement_index += 1
            x_idx = x_idx.long()

        assert x_idx.max() < n and abs(x_idx.min()) < n, "fps error"

        assert x_idx.shape == torch.Size([int(n * self.radio)]), "sampling error"
