...
def chamfer_dist_repulsion(p1, p2, k: int = 4, return_proportion: bool = False):
    ...
    # Calculate graph
    batch1 = torch.repeat_interleave(torch.arange(b1), n).type(torch.long).to(p1_.device)
    batch2 = torch.repeat_interleave(torch.arange(b2), m).type(torch.long).to(p1_.device)

    edge_index1 = knn_graph(p1_, k=k, batch=batch1)
    edge_index2 = knn_graph(p2_, k=k, batch=batch2)
